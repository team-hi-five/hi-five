...
emotion_analyzer = pipeline(
    ...
    model="joeddav/xlm-roberta-large-xnli"  # 범용 다국어 NLI
)

LABELS = ["행복", "슬픔", "분노", "공포", "당황"]
...
class OpenAIVoiceReactAgent(BaseModel):
    ...
    _send_event: Callable[[dict], Coroutine[Any, Any, None]] = PrivateAttr()
    ...
    async def aconnect(
        ...
        async with connect(
            ...
            # 1) aconnect()가 성공적으로 연결된 시점에 model_send를 바인딩
            self._send_event = model_send

            # 2) session.update 먼저 전송(도구/명령어/Whisper 설정)
            tool_defs = [
                {
                    "type": "function",
                    "name": tool.name,
            ...
            # 4) 이후부터는 Amerge로 입력/출력/툴을 묶어서 계속 처리
                ...
                if stream_key == "input_mic":
                    ...
                    if t == "conversation.item.input_audio_transcription.completed":
                        ...
                        if not user_text.strip():
                            print("empty user_text, skip emotion analysis")
                        else:
                            # 여기서 감정 분석
                            analysis = emotion_analyzer(
                                user_text,
                                candidate_labels=LABELS,
                                multi_label=True  # 감정이 여러 개 섞여있을 수 있으므로
                            )
                            scores = dict(zip(analysis["labels"], analysis["scores"]))

                            print("[감정 분석]", user_text)
                            print(" >>", scores)

                            # 예: {'행복': 0.78, '당황': 0.1, '분노': 0.05, ...}
    ...
    async def model_send_event(self, event: dict):
        """aconnect 이후, 외부에서 임의 이벤트를 모델에 직접 보내고 싶을 때 사용."""
        if not hasattr(self, "_send_event"):
            raise RuntimeError("No active connection to the model.")
